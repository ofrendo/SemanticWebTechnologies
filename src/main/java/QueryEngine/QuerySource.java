...
//		}
	...
	private void querySource(List<NamedEntity> entities, List<String> properties) {
		...
		//TODO: Union is potential for query parallelization -> is virtuoso parallelizing? -> if not: start query per et in threads??? 
...
//		}
		...
		queryString = "CONSTRUCT { ?s ?p ?o } WHERE { { " + String.join(" UNION ", parts) + " } }" ;
		
		// execute Query 
		model.add(execConstruct(queryString));
		stop = System.nanoTime();
		System.out.println(source + ": Queried properties and relations. Model size: " + model.size() + ". Time: " + TimeUnit.NANOSECONDS.toMillis(stop-start) + "ms");
		start = stop;
