...
\subsection{Server} 
\label{sec:server}
%OLLI: technical terms with textit{GET /RetrieveAvailableProperties}? Reference to Github: Maybe just Referencing to Repository (like in footnote 3 for OWL file)? So we can specify Repository link in one place or send it via mail. Maybe qualified cite instead of link for heroku? Or did I cite too much?
...
%OLLI: Inhaltlich ist der letzte Teilsatz nicht ganz korrekt: Momentan werden alle möglichen properties je entity abgefragt


\paragraph{Visualization}
The second API call (POST /RetrieveTriples) retrieves a set of context triples as described in section \ref{sec:sparqlQueries}. These are visualized to show the user the connections between entities. The visualization is generated by D3.js \footnote{\url{https://d3js.org/}} and a force layout. %OLLI: Beim Server sprichst du auch von zwei Calls, wobei der erst für available properties ist und der zweite das Ergebnis liefert - wäre der tripple call dann nicht der 3. call?
